...
public class AddSimulation extends MainSimulation{
	...
	static List<String> varsAndConsts;
	static boolean foundVarOrConst;
	static List<Pair<String,Integer>> varsAndConstsToAdd;
	static int methodStart;
	static List<Method> methods;
	static Method currentMethod;
	static int currentMethodNumber;
			...
			for(Method m: methods) {
				if(m.getCurrentLine() != null) {
					if(currentMethod.getCurrentLine().contains(m.getName() + "(")) {
						while(m.getCurrentLine().contains("/") || m.getCurrentLine().contains("*") || m.getCurrentLine().contains("{")) {
							
							currentOutput.set(m.getCurrentLineNumber(), m.getCurrentLine());
							m.getNextLine();
							if(m.getCurrentLine() == null || (m.getCurrentLine().contains("{"))) {	
								currentOutput.set(m.getCurrentLineNumber(), m.getCurrentLine());
	 ...
	 * 
	 * Add variables and Constants to output when they occur in a line
	 * 
	 * 
	 * @param lineToCheck
	 */
